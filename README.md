# Go Wind UBA 风行用户行为分析系统

Go Wind UBA 是一个面向企业的用户行为分析系统，支持多租户、事件采集、行为建模、风险分析、数据同步（ClickHouse/Doris）、多维度报表与可视化。

## 系统架构

- 前端：Vue3 + TypeScript + Ant Design Vue，提供交互式仪表盘、报表、分析视图。
- 后端：Go + Kratos，支持 REST/gRPC，事件采集、数据同步、分析模型、权限管理。
- 数据存储：支持 ClickHouse、Doris，自动同步 schema，支持高性能分析。
- 数据流：事件采集 → 数据同步 → 分析建模 → 报表展示。

## 快速启动

1. 安装 Go 1.20+、Node.js、Docker（可选）
2. 克隆项目：`git clone ...`
3. 后端依赖安装：
   ```shell
   cd backend
...
5. 启动服务：
   ...
   make run
   ```
6. 配置数据库（ClickHouse/Doris），参考 `backend/sql/` 目录。
...
- 用户行为事件采集与存储
- 多租户支持
- 数据同步（ClickHouse → Doris）
- Schema 自动同步与类型优化
- 分析模型：事件分析、漏斗分析、留存分析、归因分析、分布分析、路径分析、分群、点击分析、属性分析、行为序列分析
- 风险事件检测与建模
- 多维度报表与可视化
- 权限与角色管理
...
- 支持 ClickHouse 与 Doris 的 schema 自动同步，字段、分区、索引、主键等保持一致。
- 支持 struct 定义自动生成、注解（json、ch）自动处理。
- 支持批量数据同步与插入，严格模式下自动补齐 NOT NULL 字段。

## 常见问题 FAQ

- 插入数据时 event_date 字段为 null，Doris 严格模式下会报错。请确保 event_date 字段有值或调整表结构允许 null。
- 目录下 package 声明需统一，否则会编译报错。
- 数据同步后如需优化字段类型、索引、分区等，可参考 `backend/sql/` 脚本。
