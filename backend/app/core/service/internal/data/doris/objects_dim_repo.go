...
type ObjectsDimRepo struct {
	...
	repository *dorisCrud.Repository[ubaV1.ObjectDim, schema.ObjectsDim]
}
...
func NewObjectsDimRepo(
	...
	repo := &ObjectsDimRepo{
		log:       ctx.NewLoggerHelper("objects-dim/doris/repo/core-service"),
		db:        db,
...
func (r *ObjectsDimRepo) init() {
	r.repository = dorisCrud.NewRepository[ubaV1.ObjectDim, schema.ObjectsDim](
		r.db,
		r.mapper,
		r.tableName,
		r.log,
