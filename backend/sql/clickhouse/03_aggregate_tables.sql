...
(
    String
) COMMENT '事件大类（auth 认证/pay 支付/game 游戏/content 内容/security 安全）',
    event_name LowCardinality
(
    String
) COMMENT '事件名称（login 登录/level_up 升级/purchase 购买/click 点击）',
    platform LowCardinality
(
    String
) COMMENT '平台类型（iOS/Android/Web/H5/小程序）',
    country LowCardinality
(
    String
) COMMENT '国家/地区（用于国际化分析）',
    channel LowCardinality
(
    String
) COMMENT '渠道（app_store/website/third_party 等，用于渠道分析）',

    -- ========== 核心指标（使用 AggregateFunction 支持精确去重）==========
    uv AggregateFunction
(
    uniqCombined,
    UInt32
) COMMENT '去重用户数（使用 uniqCombined 状态函数，支持精确去重）',
    pv SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '事件总数（页面浏览量/事件量，直接累加）',
    session_count AggregateFunction
(
    uniqCombined,
    UInt64
) COMMENT '会话总数（独立会话数量）',
    ...
    -- ========== 业务指标 ==========
    total_amount SimpleAggregateFunction
(
    sum,
    Decimal
(
    38,
    2
)) COMMENT '总金额（充值/订单金额总和，Decimal(38,2) 防止溢出）',

    -- 平均时长（用 sum + count 代替 avg，因为 SimpleAggregateFunction 不支持 avg）
    duration_sum SimpleAggregateFunction
(
    sum,
    Float64
) COMMENT '时长总和（用于计算平均时长）',
    duration_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '时长计数（用于计算平均时长）',

    risk_event_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '风险事件数（risk_level != normal 的事件数量）',

    -- ========== 游戏特有指标（示例）==========
    level_up_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '升级次数（event_name = level_up 的事件数量）',
    pay_user_count AggregateFunction
(
    uniqCombined,
    UInt32
) COMMENT '付费用户数（amount > 0 的去重用户数）'
    )
...
(
    String
) COMMENT '平台类型（iOS/Android/Web/H5/小程序，用于分平台分析）',

    -- ========== 核心指标 ==========
    session_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '会话总数',
...
(
    ...
    bounce_sum SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '跳出次数总和（is_bounce = 1 的累加）',
    bounce_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '跳出计数（用于计算跳出率）',
...
(
    ...
    p50_duration AggregateFunction
(
    quantileTiming
(
    0.5
), UInt64) COMMENT '50 分位时长（中位数，50% 用户会话时长低于此值）',
    p90_duration AggregateFunction
(
    quantileTiming
(
    0.9
), UInt64) COMMENT '90 分位时长（90% 用户会话时长低于此值）',
    p99_duration AggregateFunction
(
    quantileTiming
(
    0.99
), UInt64) COMMENT '99 分位时长（99% 用户会话时长低于此值，用于识别异常长会话）'
    )
...
(
    String
) COMMENT '风险类型（login_anomaly 登录异常/fraud_payment 欺诈支付/brute_force 暴力破解）',
    risk_level LowCardinality
(
    String
) COMMENT '风险等级（normal/suspicious/high/critical）',
    status LowCardinality
(
    String
) COMMENT '处置状态（pending 待处理/confirmed 已确认/false_positive 误报/ignored 已忽略）',

    -- ========== 计数指标 ==========
    event_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '风险事件总数',
...
(
    uniqCombined,
    UInt32
) COMMENT '去重用户数（触发风险的去重用户）',

    confirmed_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '已确认风险数（status = confirmed 的事件数量）',

    -- 平均风险分（用 sum + count 代替 avg）
    risk_score_sum SimpleAggregateFunction
(
    sum,
    Float64
) COMMENT '风险分总和（用于计算平均风险分）',
    risk_score_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '风险分计数（用于计算平均风险分）'
    )
    ENGINE = AggregatingMergeTree -- 聚合树引擎
    PARTITION BY toYYYYMM
(
...
(
    ...
    UInt32
    COMMENT
    '标签定义 ID（关联 uba_tag_definitions.id）',
    tag_value
    String
    COMMENT
    '标签值（用于按标签值分群）',
...
(
    uniqCombined,
    UInt32
) COMMENT '标签用户数（拥有该标签的去重用户数）',
    sample_users AggregateFunction
(
    groupArraySample
(
    1000
), UInt32) COMMENT '抽样用户 ID 数组（用于运营预览，最多 1000 个用户）'
    )
    ENGINE = AggregatingMergeTree -- 聚合树引擎
    PARTITION BY toYYYYMM
(
...
(
    String
) COMMENT '事件序列数组（如["login", "browse", "cart"]）',
    sequence_hash String COMMENT '序列哈希值（用于去重和快速匹配）',

    -- ========== 统计指标 ==========
    support_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '支持度（该路径出现的次数）',
    unique_users AggregateFunction
(
    uniqCombined,
    UInt32
) COMMENT '去重用户数（走该路径的去重用户）',
...
(
    ...
    conversion_sum SimpleAggregateFunction
(
    sum,
    Float64
) COMMENT '转化次数总和（is_converted = 1 的累加）',
    conversion_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '转化计数（用于计算转化率）'
    )
    ENGINE = AggregatingMergeTree -- 聚合树引擎
    PARTITION BY toYYYYMM
(
...
(
    ...
    stat_date,
    sequence_hash
) -- 按租户 + 日期 + 序列哈希排序（不能用 support_count，聚合字段不能在 ORDER BY 中）
...
(
    ...
    user_level LowCardinality
(
    String
),

    active_users AggregateFunction
(
    uniqCombined,
    UInt32
),
    --new_users      AggregateFunction(uniqCombined, UInt32),
    pay_users AggregateFunction
(
    uniqCombined,
    UInt32
),
    risk_users AggregateFunction
(
    uniqCombined,
    UInt32
),

    total_sessions AggregateFunction
(
    uniqCombined,
    UInt64
),
    total_events SimpleAggregateFunction
(
    sum,
    UInt64
)
...
(
    ...
    pay_level LowCardinality
(
    String
) COMMENT '付费等级（free/paying/vip）',
...
(
    ...
    pay_order_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '订单数（付费事件数）',
...
(
    ...
    -- 退款相关
    refund_count SimpleAggregateFunction
(
    sum,
    UInt64
) COMMENT '退款订单数',
    refund_amount SimpleAggregateFunction
(
    sum,
    Decimal
(
...
(
    ...
    enter_count SimpleAggregateFunction
(
    sum,
    UInt64
),
    exit_count SimpleAggregateFunction
(
    sum,
    UInt64
)
...
(
    ...
    enter_users AggregateFunction
(
    uniqCombined,
    UInt32
),
    complete_users AggregateFunction
(
    uniqCombined,
    UInt32
)
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_sessions_agg_daily
...
SELECT tenant_id,
       ...
       -- 分位数（使用 State 函数）
       quantileTimingState(0.5)(duration_ms)  as p50_duration, quantileTimingState(0.9)(duration_ms)  as p90_duration, quantileTimingState(0.99) (duration_ms) as p99_duration
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_risk_stats_daily
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_user_tags_agg
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_popular_paths_daily
    TO gw_uba.popular_paths_daily
AS
SELECT tenant_id,
       event_date as stat_date, [first_event, arrayElement(first_3_events, 2), arrayElement(first_3_events, 3)] as event_sequence, cityHash64(event_sequence) as sequence_hash, count () as support_count, uniqCombinedState(user_id) as unique_users, sum (total_duration_ms) as duration_sum, count () as duration_count, sum (is_converted) as conversion_sum, count () as conversion_count
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_events_agg_daily
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_user_activity_daily
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_pay_agg_daily
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_page_visit_daily
...
SELECT tenant_id,
       ...
       sum(total_amount)                       AS total_amount,
       quantileTimingMerge(0.5)(p50_duration)  AS p50_duration, quantileTimingMerge(0.9)(p90_duration)  AS p90_duration, quantileTimingMerge(0.99) (p99_duration) AS p99_duration
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_events_agg_hourly
...
-- -----------------------------------------------------------
CREATE
MATERIALIZED VIEW IF NOT EXISTS gw_uba.mv_funnel_steps_daily
