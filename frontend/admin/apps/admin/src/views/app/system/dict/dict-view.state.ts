...
import { fetchListDictEntries, fetchListDictTypes, fetchListLanguages, PaginationQuery } from '#/api';
...
export const useDictViewStore = defineStore('dict-view', {
  ...
  actions: {
    ...
    async fetchTypeList(
      ...
      this.loading = true;
      try {
        this.typeList = await fetchListDictTypes(new PaginationQuery({ paging: { page: currentPage, pageSize }, formValues: formValues, }));
    ...
    async fetchEntryList(
      ...
      try {
        this.entryList = await fetchListDictEntries(
          new PaginationQuery({
            paging: { page: currentPage, pageSize },
            formValues: { ...formValues, type_id: typeId.toString() },
          }),
        );
      } catch (error) {
